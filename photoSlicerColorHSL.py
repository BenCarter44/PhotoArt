...
def toHLS(color):
    sampleImg = np.ones((1,1,3), dtype=np.uint8)
    sampleImg[0,0] = color
   # print(sampleImg)
    hls = cv2.cvtColor(sampleImg,cv2.COLOR_BGR2HLS) # h is now RED. l is now GREEN. s is now BLUE
  #  print(colr)

    hlsAdj = hls[0,0]
    return [mapValue(hlsAdj[0],0,180,0,255),hlsAdj[1],hlsAdj[2]]

def toBGR(color):
    sampleImg = np.ones((1,1,3), dtype=np.uint8)
    sampleImg[0,0] = [mapValue(color[0],0,255,0,180),color[1],color[2]]
   # print(sampleImg)
    hls = cv2.cvtColor(sampleImg,cv2.COLOR_HLS2BGR) # h is now RED. l is now GREEN. s is now BLUE
  #  print(colr)
    return hls[0,0]
...
def colorMatch(color,colormap):
  ...
  #  print(lowBlue,lowGreen,lowRed)
    ...
    for x in matrix:
        ...
        i = color[2] * 0.299 + color[1] * 0.587 + color[0] * 0.114
        if(i < grayValLow):
            grayValLow = i
        if(i > grayValHigh):
            grayValHigh = i
...
#
 ...
 #       txtNum = weights.index(adjustedValue)
    ...
    olB = backOut
    backOut = toBGR(backOut)
    olT = textColorOut
    textColorOut = toBGR(textColorOut)
    tester = toBGR(olB)
    
  #  print(backOut,textColorOut,olB,olT,tester)
...
for y in range(height):
    for x in range(width):
     ...
     #   debug += str(out[0]) + "," + str(out[1]) + " "
        ...
        dump = str(int(out[0])) + " " + str(int(txtColor[2])) + " " + str(int(txtColor[1])) + " " + str(int(txtColor[0])) + " " + str(int(backColor[2])) + " " + str(int(backColor[1])) + " " + str(int(backColor[0])) + " "
        f.write(dump)
        pbar.update(1)
    dump = ""
    #print(debug)
